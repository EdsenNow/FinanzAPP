const TABLE_CURRENCY_FORMATTER = new Intl.NumberFormat('es-ES', {
  minimumFractionDigits: 2,
  maximumFractionDigits: 2
});

const TABLE_CURRENCY_FORMATTER_NO_CENTS = new Intl.NumberFormat('es-ES', {
  minimumFractionDigits: 0,
  maximumFractionDigits: 0
});

function getTableFormatter() {
  return window.__appShowCents === false ? TABLE_CURRENCY_FORMATTER_NO_CENTS : TABLE_CURRENCY_FORMATTER;
}
...
class GestorTablas {
  constructor(tableId, options = {}) {
    ...
    this.config = {
      itemsPerPage: window.__appTxPerPage || 10,
      currentPage: 1,
      sortKey: 'name',
      sortDirection: 'asc',
      ...options
  ...
  actualizarTabla() {
    ...
    this.paginatedData.forEach(item => {
      const row = document.createElement('tr');
      const formattedAmount = window.Core?.helpers?.formatCurrency
        ? window.Core.helpers.formatCurrency(item.amount)
        : getTableFormatter().format(Number(item.amount) || 0);
      
      const isExpenseTable = this.table.id === 'expensesTable' || 

 ...
 * Orquestado por `DashboardApp extends BasePage` al final del archivo.
 *
 ...
 * @param {{ mensaje: string, confirmText?: string, cancelText?: string, variant?: string, onConfirm: Function }} param0
 */
...
function limpiarTransaccionesCategoria(categoryId, triggerEl = null) {
  ...
  mostrarBottomDrawer({
    mensaje: `¿Limpiar todas las transacciones de "<strong>${category.name}</strong>"?`,
    confirmText: 'Limpiar',
    cancelText: 'Cancelar',
    variant: 'danger',
    onConfirm: ejecutar
...
function renderizarCategorias() {
  ...
  tarjeta.className = `category-card fade-in${category.isPinned ? ' pinned' : ''}${isExpanded ? '' : ' mobile-collapsed'}`;
...
function mostrarModalEliminarCategoria(categoryId) {
  ...
  mostrarBottomDrawer({
    mensaje: `¿Eliminar la categoría "<strong>${category.name}</strong>"?<br>Se eliminarán todas sus transacciones.`,
...
function configurarListenersEventos() {
  ...
  if (logoutButton) {
    logoutButton.addEventListener('click', async function(e) {
      ...
      try {
        // Cerrar sesión en Firebase (esto establece el logoutTimestamp)
        if (window.firebaseAuth) {
          await window.firebaseAuth.logout();
        } else {
          // Si Firebase no está disponible, limpiar almacenamiento y establecer timestamp
        ...
        // Aun así, continuar redirigiendo al login: eliminar claves de sesión y preservar timestamp
        const ts = Date.now().toString();
  ...
  if (clearAllBtn) clearAllBtn.addEventListener('click', () => {
    ...
    if (esVistaEscritorio()) {
      mostrarBottomDrawer({
        mensaje: '¿Quieres eliminar todas las categorías y sus transacciones?<br>Esta acción no se puede deshacer.',
        confirmText: 'Eliminar',
        cancelText: 'Cancelar',
        variant: 'danger',
 ...
 *
 * `DashboardApp` extiende `BasePage` para heredar la verificación de autenticación
...
const _dashboardApp = new DashboardApp();

...
const GMAIL_CLIENT_ID = '569331846575-djonqen9ib9jrek93o0hpjem189ppjsm.apps.googleusercontent.com';
...
class GmailAPI {
  ...
  _signInWithTokenClientDirectly() {
    return new Promise((resolve, reject) => {
      let settled = false;
      const tokenClient = window.google.accounts.oauth2.initTokenClient({
        client_id: this._clientId,
        ...
        callback: (response) => {
          ...
          settled = true;
          if (response.error || !response.access_token) {
            reject(new Error(response.error_description || response.error || 'error_oauth'));
            return;
          }
          resolve(response);
        },
        error_callback: (err) => {
          if (settled) return;
          settled = true;
          reject(new Error(err?.message || err?.type || 'popup_closed'));
        }
      });
  ...
  /**
   * Abre la ventana emergente OAuth de Google para conectar Gmail.
   * Se ejecuta de forma sincrónica en el clic del usuario para evitar que el navegador bloquee el popup.
   * @returns {Promise<void>}
   */
  signInInteractive() {
    return new Promise((resolve, reject) => {
      if (!window.google?.accounts?.oauth2) {
        this._waitForGis().then(() => {
          this.signInInteractive().then(resolve).catch(reject);
        }).catch(reject);
        return;
      }
      ...
      const handleSuccess = (accessToken, expiresIn) => {
        ...
        this._saveToken(accessToken, expiresIn || 3600);
        this._signedIn = true;
        if (this._onStatusChange) this._onStatusChange(true);
        resolve();
      };
      ...
      const handleError = (error) => {
        ...
        clearTimeout(timeout);
        this._signedIn = false;
        let msg = typeof error === 'string' ? error : (error?.error_description || error?.error || error?.message || 'error_oauth');
        if (msg === 'popup_closed_by_user' || msg === 'user_closed_popup' || error?.type === 'popup_closed') {
          msg = 'popup_closed';
        } else if (msg === 'popup_failed_to_open' || /failed.*open.*popup/i.test(msg)) {
          msg = 'popup_blocked';
        }
        reject(new Error(msg));
      };
  ...
  startPolling(onTransaction) {
    ...
    this.stopPolling();
    this.scanRecentEmails();
    this._poll();
    this._pollingTimer = setInterval(() => this._poll(), GMAIL_POLLING_INTERVAL_MS);
  }

  /**
   * Escanea los correos recientes en la bandeja para detectar notificaciones bancarias (incluyendo reenviados).
   */
  async scanRecentEmails() {
    if (!this.isSignedIn()) return [];

    try {
      const query = encodeURIComponent('Popular OR Qik OR BHD OR Banreservas OR "Banco Popular" OR "Aprobada" OR banco OR transaccion OR compra OR pago OR RD$ OR US$');
      ...
      for (const m of messages) {
        if (this._processedIds.has(m.id)) continue;
        ...
        if (parsed && !parsed.ignored) {
          console.log('[GmailAPI] Transacción escaneada:', parsed);
      ...
      console.error('[GmailAPI] Error escaneando correos recientes:', e);
      return [];
    }
  }
  ...
  _parseMerchant(text) {
    ...
    const patterns = [
      // Qik / BHD / Popular / Banreservas: "Localidad UBER*EATS" o "Comercio: UBER*EATS"
      /(?:localidad|comercio|merchant|establecimiento|local)[:\s]+([A-Za-z0-9 &'.,:/\-*]{2,60})/i,
      // "transacción de RD$ 413.68 en UBER*EATS con tu Tarjeta..."
    ...
    for (const p of patterns) {
      ...
      // Limpiar solo si la red repite exactamente el mismo nombre (ej: "DIDI RIDES-W*DIDI RIDES-W")
